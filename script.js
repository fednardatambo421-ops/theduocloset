...
const receiptKey = 'duo-closet-receipts';
const sixMonths = 183 * 24 * 60 * 60 * 1000;
const getReceipts = () => { try { const valid = JSON.parse(localStorage.getItem(receiptKey) || '[]').filter((receipt) => Date.now() - new Date(receipt.date).getTime() < sixMonths); localStorage.setItem(receiptKey, JSON.stringify(valid)); return valid; } catch { return []; } };
const saveReceipt = (receipt) => localStorage.setItem(receiptKey, JSON.stringify([receipt, ...getReceipts()]));
...
document.querySelector('.checkout-form').addEventListener('submit', (event) => {
  ...
  const items = [...bag]; const total = items.reduce((sum, item) => sum + item.price, 0);
  const order = `TDC-${Math.floor(100000 + Math.random() * 900000)}`;
  const receipt = { order, date: new Date().toISOString(), total, items: items.map((item) => item.name), status: 'Order received' }; saveReceipt(receipt);
  ...
  document.querySelector('.receipt-details').innerHTML = `<div><span>Order</span><strong>${order}</strong></div><div><span>Items</span><strong>${items.length}</strong></div><div><span>Total</span><strong>${money(total)}</strong></div><div><span>Delivery</span><strong>${data.get('address')}</strong></div>`;
  document.querySelector('.receipt').classList.add('open');
  document.querySelector('.receipt').setAttribute('aria-hidden', 'false');
  const orderText = `Hello The Duo Closet, I would like to place order ${order}.%0AName: ${encodeURIComponent(data.get('name'))}%0APhone: ${encodeURIComponent(data.get('phone'))}%0ADelivery: ${encodeURIComponent(data.get('address'))}%0AItems: ${encodeURIComponent(items.map((item) => item.name).join(', '))}%0ATotal: ${encodeURIComponent(money(total))}`;
  window.open(`https://wa.me/254715067628?text=${orderText}`, '_blank', 'noopener');
...
const receiptsModal = document.querySelector('.receipts-modal');
const renderReceipts = () => { const receipts = getReceipts(); document.querySelector('.receipts-list').innerHTML = receipts.length ? receipts.map((receipt) => `<article class="saved-receipt"><div><p>${receipt.order}</p><span>${new Date(receipt.date).toLocaleDateString('en-KE', { day: 'numeric', month: 'short', year: 'numeric' })} · ${receipt.status}</span></div><strong>${money(receipt.total)}</strong><a target="_blank" rel="noopener" href="https://wa.me/254715067628?text=${encodeURIComponent(`Hello The Duo Closet, I would like an update on order ${receipt.order}.`)}">Track on WhatsApp ↗</a></article>`).join('') : '<p class="no-receipts">No receipts yet. Your completed orders will appear here.</p>'; };
const setReceipts = (open) => { receiptsModal.classList.toggle('open', open); receiptsModal.setAttribute('aria-hidden', String(!open)); if (open) renderReceipts(); };
document.querySelector('.receipts-float').addEventListener('click', () => setReceipts(true));
document.querySelector('.receipts-close').addEventListener('click', () => setReceipts(false));
